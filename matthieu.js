const functionLoto = (firstname, lastname, email, loto) => {
	if (firstname) {
	...
	if (email) {
		if (email.length > 8) {
			if (email.length < 30) {
				...
				let resultat = reg_syntax_email.test(email);
				if (resultat) {
					var results = [];
...
checkLoto.addEventListener("submit", (e) => {
	...
	functionLoto(firstname.value, lastname.value, email.value, lotoNumbers);
});
